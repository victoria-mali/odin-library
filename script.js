...
function validateInput(input) {
  ...
  if (input.validity.valueMissing) {
    input.setCustomValidity(`You need to enter ${input.name}.`);
  }

  if (input.validity.tooShort) {
    input.setCustomValidity(
      `${capitalizedName} should be at least ${input.minLength} characters.`,
    );
  }
...
const inputs = [formTitle, formAuthor, formPages];
inputs.forEach(input => addValidation(input));



favDialog.addEventListener("submit", (e) => {
  if (!formTitle.validity.valid || !formAuthor.validity.valid || !formPages.validity.valid) {
    e.preventDefault();
  } else {
    favDialog.close(
      formTitle.value,
